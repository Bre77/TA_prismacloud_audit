...
- Credential handling goes through `self.context.credentials.protect_input_fields(...)` (see `bin/prismacloud_audit.py`), not manual `storage_passwords` list/delete/create calls. This preserves the credential identity `(owner=nobody, app=TA_prismacloud_audit, realm=<stanza name>, username=api_key)` that existing installs already have - do not change that tuple without a captain-level decision; it strands users' stored secrets.
